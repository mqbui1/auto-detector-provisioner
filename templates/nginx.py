...
class NginxTemplates:
    ...
    def templates(service: str, environment: str, baseline: Any | None = None) -> list[DetectorTemplate]:
        ...
        detectors.append(DetectorTemplate(
            ...
            severity="Major",
            signalflow=f"""
total = data("nginx.requests", filter={f}).sum(over="2m")
errors = data("nginx.requests", filter={f} and filter("status", "5*")).sum(over="2m")
...
""".strip(),
        ...
        detectors.append(DetectorTemplate(
            ...
            severity="Major",
            signalflow=f"""
active = data("nginx.connections.active", filter={f}).mean(over="2m")
accepted = data("nginx.connections.accepted", filter={f}).mean(over="2m")
...
""".strip(),
        ...
        detectors.append(DetectorTemplate(
            ...
            severity="Warning",
            signalflow=f"""
A = data("nginx.connections.waiting", filter={f}).mean(over="5m")
...
""".strip(),
        ...
        detectors.append(DetectorTemplate(
            ...
            severity="Major",
            signalflow=f"""
A = data("nginx.upstream.response.time", filter={f}).percentile(pct=99, over="5m")
...
""".strip(),
        ...
        detectors.append(DetectorTemplate(
            ...
            severity="Major",
            signalflow=f"""
A = data("nginx.requests", filter={f}).mean(over="5m")
hist = data("nginx.requests", filter={f}).mean(over="1h")
...
""".strip(),
        ...
        detectors.append(DetectorTemplate(
            ...
            severity="Warning",
            signalflow=f"""
total = data("nginx.requests", filter={f}).sum(over="5m")
client_errors = data("nginx.requests", filter={f} and filter("status", "4*")).sum(over="5m")
