...
""".strip(),
            ...
            rationale=(
                ...
                "High GC rates indicate excessive object creation/retention, a precursor to memory leaks "
                "or OOM. Gen-0 collections are cheap; Gen-2 (full) collections cause latency spikes. "
                "Metric: process.runtime.cpython.gc_count from OTel Python runtime instrumentation. "
                "Thresholds: 10/min (warn) / 50/min (critical) from Python memory profiling best practices. "
                "Tune by checking which generation is spiking with gc.get_stats()."
            ),
        ))
...
""".strip(),
            ...
            rationale=(
                ...
                "Steadily growing RSS without a ceiling indicates a memory leak. Python leaks commonly "
                "occur via global caches, event listeners never unregistered, or C extensions holding refs. "
                "Metric: process.memory.rss from OTel Python system-metrics (OTel semantic conventions). "
                "Thresholds: 512MB (warn) / 1GB (critical) — conservative defaults for container workloads. "
                "Tune to your container memory limit (typically warn at 70%, critical at 90%)."
            ),
        ))
