...
class JVMTemplates:
    ...
    def templates(service: str, environment: str, baseline: Any | None = None) -> list[DetectorTemplate]:
        ...
        detectors.append(DetectorTemplate(
            ...
            severity="Major",
            signalflow=f"""
used = data("jvm.memory.heap.used", {f}).mean(over="5m")
max_heap = data("jvm.memory.heap.max", {f}).mean(over="5m")
...
""".strip(),
        ...
        detectors.append(DetectorTemplate(
            ...
            severity="Major",
            signalflow=f"""
A = data("jvm.gc.pause", {f}).mean(over="5m")
        ...
        if baseline and "jvm.gc.collections.count" in baseline.metrics:
            ...
            if m.is_reliable():
                ...
                detectors.append(DetectorTemplate(
                    ...
                    severity="Warning",
                    signalflow=f"""
A = data("jvm.gc.collections.count", {f}).rate(over="1m")
...
""".strip(),
        ...
        detectors.append(DetectorTemplate(
            ...
            severity="Major",
            signalflow=f"""
A = data("jvm.threads.count", {f}).mean(over="5m")
detect(when(A > 500), lasting="5m").publish("Critical")
detect(when(A > 200) and when(A <= 500), lasting="5m").publish("Warning")
""".strip(),
            threshold_type="fixed",
        ...
        detectors.append(DetectorTemplate(
            ...
            severity="Warning",
            signalflow=f"""
A = data("jvm.memory.nonheap.used", {f}).mean(over="5m")
...
""".strip(),
        ...
        detectors.append(DetectorTemplate(
            ...
            severity="Major",
            signalflow=f"""
active = data("hikaricp.connections.active", {f}).mean(over="2m")
max_pool = data("hikaricp.connections.max", {f}).mean(over="2m")
...
""".strip(),
        ...
        detectors.append(DetectorTemplate(
            ...
            severity="Major",
            signalflow=f"""
busy = data("tomcat.threads.busy", {f}).mean(over="2m")
max_t = data("tomcat.threads.config.max", {f}).mean(over="2m")
